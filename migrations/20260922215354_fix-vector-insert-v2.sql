...
BEGIN
  -- Convert JSON array to vector: JSON array [0.1, 0.2, ...] -> text '[0.1,0.2,...]' -> vector
  -- Use jsonb_array_elements with ordinality to maintain order
  v_embedding_vector := (
    SELECT ('[' || string_agg(elem, ',') || ']')::VECTOR(1536)
